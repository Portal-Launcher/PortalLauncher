...
- **Bring your instances with you.** Add Instance > Other Launchers finds what you already have in the Minecraft Launcher, CurseForge, the Modrinth App, MultiMC, PolyMC, Prism Launcher, GDLauncher, ATLauncher, Technic and XMCL, and imports it with mods, worlds and settings. The other launcher keeps its copy.
...
- Windows SmartScreen may warn because the build is not code signed yet: click "More info", then "Run anyway". If you would rather check the download than trust it, see [Security and privacy](#security-and-privacy) below: every file is built in public and can be verified against this repository.
